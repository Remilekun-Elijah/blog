...
function createPostHandler (req, res) {
 ...
 if(newPost.title && newPost.content) {
  ...
  res.status(400).json({success: false, message: "title and content field are required"})
 }
...
function getPostsHandler (req, res) {
 ...
 res.status(200).json({message: "Post retrieved successfully", data: Database.post})
}
